...
* Investigate Bayesian modelling techniques on shooting data, and to learn more about time-dependency and the "hot hand" concept in sports.
...
plotshottime <- function(playerid, season, main, showstats = FALSE){
  ...
  if(showstats){
    stats <- loess.smooth(X,Y,span=20/length(Y), main = main,
                   yaxt="n",ylab="Result",xlab="Order")
    print(c(nrow(Xtot_sub), length(stats$y)))
  }
}
...
for(i in 1:nrow(playerseasons)){
  r <- playerseasons[i,]
  plotshottime(r[[1]],r[[2]], paste0("Player ",i), showstats=TRUE)
}
```

## Exploratory Data Analysis (cont.)

```{r smoothplot, fig.cap = "Loess Smoothing Curve on Shot Success Rate", fig.height=5}
...
```{r glmplot, fig.cap = "Posterior Distributionsof GLM Parameters", out.width="90%", fig.align="right", cache=TRUE}
glmplot5
```
...
```{r hierplot, fig.height = 5, fig.cap = "Median Player Effects over Population Distribution"}
...
```{r contplot, fig.cap = "Contour Plots", out.width="90%", fig.align="right"}
contplotteam
```
...
```{r discplot, fig.height=5, fig.cap = "Discount Weights by $\\delta$ and Time Difference"}
...
plot_time_effect <- function(game.params = NA, ids = NA, parameters = NA, main=NA, zlab=NULL, only_fga_games = TRUE, yrange = c(NULL,NULL)){
  ...
  if(length(ids) == 1){
    ...
    ggplot(data = plotdat, 
      ...
      labs(title=main, 
           y="Estimate", 
           x=expression(Anchor ~ Game ~ Index ~ (g[0])), 
           color = "Parameters") +
...
plot_calibration <- function(dat, title="Calibration Plot", xlab=NA){
  ...
  ggplot(data = dat, aes(x=as.numeric(as.character(bin)), y=p, group=0)) + 
    ...
    theme_bw() + 
    labs(x=xlab,y="Actual Probability" , title=title)
}
...
calitrain <- Xtrain$result %>% 
  ...
  plot_calibration(., "Training Set", "Fitted Probabilities")

calitest <- Xtest$result %>% 
  calibrate(observed=., predicted=pred.game.850) %>% 
  plot_calibration(., "Testing Set", "Predicted Probabilities")
