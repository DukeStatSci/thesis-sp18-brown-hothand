...
kabsum[,"season"] <- c("categorical", "{2014, 2015, 2016, 2017}", "")
kabsum[,"gameid"] <- c("categorical", "NA", "")
kabsum[,"time"] <- c("continuous", "NA", "13-digit timestamp in milliseconds")
kabsum[,"globalplayerid"] <- c("categorical", "NA", "")
...
ggplot(Xtotrange, aes(x=xt, y=yt, color=theta)) + 
  annotation_custom(rasterGrob(img,
                               x=0.50, y=0.50,
                               width = .92, height = .92),
                    -Inf, Inf, -Inf, Inf) + 
  scale_color_gradient2(low = rgb(0,0,0), # black
                       mid = rgb(0,0,1), # blue 
                       high= rgb(1,1,1), # white 
                       midpoint=-0.15) +
  geom_point() +
  geom_vline(xintercept=0) +
  labs(title = "Distribution of Shot Locations", y="", x="") +
  theme_bw() + 
...
kable(t(kabsum))
kable(kab)
# TODO: illustration of the court marking where is theta > 0, 
```
