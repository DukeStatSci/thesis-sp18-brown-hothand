...
priormod <- glm(result ~ log(r) + theta, data=Xtrainsub, family="binomial")
...
fit_glm <- function(dat, S = 10000, B = 500){
  ...
  model.glm <- function(){
    ...
    # because shot success rate should get worse as distance increases under baseline circumstances.
...
fit_players <- function(dat = NA, S = 10000, B = 500){
  ...
  # we want posteriors for the overall effects and the individual player effects
  params <- c("beta_int", 
              "beta_home", 
              "beta_r", 
              "beta_theta",
  ...
  awaytext <- paste0("`beta_intA[",1:M,"]` = `beta_int[",1:M,"]`", 
                     collapse=",\n")
