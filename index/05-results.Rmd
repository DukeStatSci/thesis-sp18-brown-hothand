...
Xtrain <- allgameshots[-testrows,]
Xtest <- allgameshots[testrows,]
...
load(file="../rdatafiles/glmtot.RData")
params.team <- apply(select(glmtot, -deviance), 2, mean) %>% 
  t() %>%
  as.data.frame() %>%
...
p.team <- pred_shot(Xtest, params.team)
p.player <- pred_shot(Xtest, params.player)
p.playergame <- pred_shot(Xtest, params.playergame)
...
# pROC::roc(allgameshots$result, p.player, plot=TRUE)
# pROC::roc(allgameshots$result, p.playergame, plot=TRUE)
