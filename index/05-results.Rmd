...
library(mvtnorm); library(dplyr); library(ggplot2); library(R2jags); library(knitr); library(reshape2); library(png); library(grid); library(gridExtra); library(stringr)
...
params.glm <- t(select(glmtot, -deviance))
pred.glm <- Xmat[,1:3] %>%
  '%*%'(params.glm) %>%
  arm::invlogit() %>%
  apply(., 1, median)
...
params.me <- player.mcmc %>% 
  mutate("s" = 1:nrow(player.mcmc)) %>%
  select(which(grepl("[", names(.), fixed=TRUE) | colnames(.) == "s")) %>%
  melt(id = "s") %>% 
  mutate(factorid = str_extract(variable, "[[:digit:]]+"),
         param    = sub(variable, pat="\\[(.*)",rep="")) %>%
  dcast(s + factorid ~ param, value.var = "value")

pred.me <- Xmat[,1:3] %>%
  '%*%'(t(matrix(as.numeric(params.me[,c(-1,-2)]), ncol=3)))

keeprows.me <- 0 #somehow filter out unnecessary rows from pred.me
