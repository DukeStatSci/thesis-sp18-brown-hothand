...
fit_glm <- function(dat, S = 10000, B = 500){
  ...
  inits.all <- list(list("beta_int"=0, "beta_r"=0, "beta_theta"=0#,
...
plot_time_effect <- function(params.season = NA, id = NA, parameter = NA, only_fga_games = TRUE){
  ...
  ggplot(data = plotdat, 
    ...
    geom_line() +   
    theme_bw() +
    labs(title=paste("Changes in Estimate for", param_map[[parameter]], "Parameter Over Time"), y="Estimate", x="Game") + 
    theme(panel.grid = element_blank())
...
g1 <- plot_time_effect(params.season.all, id=id1, parameter="int")
g2 <- plot_time_effect(params.season.all, id=id2, parameter="int")
g3 <- plot_time_effect(params.season.all, id=id3, parameter="int")
g4 <- plot_time_effect(params.season.all, id=id4, parameter="int")
g5 <- plot_time_effect(params.season.all, id=0, parameter="int")
...
# means
params1 <- apply(select(glmtot, -deviance), 2, mean) %>% 
  t() %>%
  as.data.frame() %>%
  select(beta_int, beta_r, beta_theta)

params2 <- player.params %>% 
  select(beta_int=int, beta_r=r, beta_theta=theta, globalplayerid)

params3 <- params.season.all %>%
  select(beta_int=int, beta_r=r, beta_theta=theta, globalplayerid, gameid)

# medians
params1 <- apply(select(glmtot, -deviance), 2, median) %>% 
  t() %>%
  as.data.frame() %>%
  select(beta_int, beta_r, beta_theta)
...
# parameter estimates for each combination of gameid and playerid...even if player did not play in that game.
...
conf_mat <- function(actual = NA, pred = NA, cutoff = 0.5){
...
p.team <- pred_shot(Xtest, params1)
p.player <- pred_shot(Xtest, params2)
p.playergame <- pred_shot(Xtest, params3)


conf_mat(Xtest$result, p.team)
conf_mat(Xtest$result, p.player)
conf_mat(Xtest$result, p.playergame)
...
#investigative work on poor predictions. I think it is be because of the order of the shots after the merge

X <- merge(Xtest, params1) %>% 
  merge(params2, by=c("globalplayerid"), suffix=c("","2")) %>% 
  merge(params3, by=c("globalplayerid", "gameid"), suffix=c("","3")) %>%
  rename(beta_int1 = beta_int, beta_r1 = beta_r, beta_theta1 = beta_theta) %>%
  mutate(
    Y1 = beta_int1 + log(r)*beta_r1 + theta*beta_theta1,
    Y2 = beta_int2 + log(r)*beta_r2 + theta*beta_theta2,
    Y3 = beta_int3 + log(r)*beta_r3 + theta*beta_theta3,
    P1 = exp(Y1)/(1+exp(Y1)),
    P2 = exp(Y2)/(1+exp(Y2)),
    P3 = exp(Y3)/(1+exp(Y3)),
    Y = result
...
par(mfrow=c(3,1))
hist(X$P1, breaks=30) #why is the distribution of probabilities so wierd?
hist(X$P2, breaks=30)
hist(X$P3, breaks=30)
par(mfrow=c(1,1))
