...
# TODO:
  # I think the likelihood is all messed up because that should be used to compare nested models...right?
...
rdatafiles <- paste0("../rdatafiles",k0,"/")
id1 <- 887665 
id2 <- 842301 
id3 <- 603106 
id4 <- 601140 
...
```{r logliks}
#theoretically, likelihood should increase for each one
m1 <- glm(result ~ 1, data=Xtrain, family=binomial)
m2 <- glm(result ~ r, data=Xtrain, family=binomial)
m3 <- glm(result ~ r + theta, data=Xtrain, family=binomial)
m4 <- glm(result ~ r + globalplayerid, data=Xtrain, family=binomial)
m5 <- glm(result ~ r + theta + globalplayerid, data=Xtrain, family=binomial)
...
For the remainder of this paper, we will select the 
