...
library(mvtnorm); library(dplyr); library(ggplot2); library(R2jags); library(pROC); library(knitr); library(reshape2); library(png); library(grid)
...
get_player_params <- function(dat=NA, sim.mcmc.hier=NA){
  ...
  playermapparams <- playermapshots %>% 
  ...
  for(i in 1:nrow(playermapshots)){
    ...
    playermapparams[i,8:ncol(playermapparams)] <- theta_i
  }
...
# save(player.mcmc, file=paste0(datafolder, "/playermcmc.RData"))
load(paste0(datafolder, "/playermcmc.RData"))
...
plot_contours <- function(sim.mcmc = player.mcmc, j = NA, MLE=FALSE){
  all_x <- seq(-25,25, 1)
  ...
  img.path <- paste0(githubfolder, "/index/figure/Basketball-Court-Dimensions2.png")
...
plot_contours(player.mcmc, j=5, MLE=FALSE)
...
fit_game <- function(dat = NA, anchor = NA, S = 10000, B = 500){
...
fit_games <- function(dat = NA, S = 10000, B = 500){
...
# DATA
dat14 <- allgameshots %>% filter(season == 2014)
dat15 <- allgameshots %>% filter(season == 2015)
dat16 <- allgameshots %>% filter(season == 2016)
dat17 <- allgameshots %>% filter(season == 2017)


# GAME MCMC LISTS
# game.mcmc.list14 <- fit_games(dat14, 1000, 100)
# save(game.mcmc.list14, file=paste0(datafolder,"/gamemcmclist14.RData"))
load(paste0(datafolder,"/gamemcmclist14.RData"))
# 
# game.mcmc.list15 <- fit_games(dat15, 1000, 100)
# save(game.mcmc.list15, file=paste0(datafolder,"/gamemcmclist15.RData"))
load(paste0(datafolder,"/gamemcmclist15.RData"))
# 
# game.mcmc.list16 <- fit_games(dat16, 1000, 100)
# save(game.mcmc.list16, file=paste0(datafolder,"/gamemcmclist16.RData"))
load(paste0(datafolder,"/gamemcmclist16.RData"))
# 
# game.mcmc.list17 <- fit_games(dat17, 1000, 100)
# save(game.mcmc.list17, file=paste0(datafolder,"/gamemcmclist17.RData"))
load(paste0(datafolder,"/gamemcmclist17.RData"))

# SEASON PARAMETERS
params.season14 <- get_season_params(dat14,game.mcmc.list14)
save(params.season14, file=paste0(datafolder,"/paramsseason14.RData"))
# load(paste0(datafolder,"/paramsseason14.RData"))

params.season15 <- get_season_params(dat15,game.mcmc.list15)
save(params.season15, file=paste0(datafolder,"/paramsseason15.RData"))
# load(paste0(datafolder,"/paramsseason15.RData"))

params.season16 <- get_season_params(dat16,game.mcmc.list16)
save(params.season16, file=paste0(datafolder,"/paramsseason16.RData"))
# load(paste0(datafolder,"/paramsseason16.RData"))

params.season17 <- get_season_params(dat17,game.mcmc.list17)
save(params.season17, file=paste0(datafolder,"/paramsseason17.RData"))
# load(paste0(datafolder,"/paramsseason17.RData"))


params.season <- params.season14
